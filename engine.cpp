...
Engine::Engine() : stopSearch(false) {
	...
	bool ok = initSyzygy("C:\\Users\\Kushang Panchal\\Desktop\\ChessGamev2\\Chess\\syzygy");
	qDebug() << "tb_init:" << ok << "largest:" << TB_LARGEST;

	loadOpeningBook("./Book/komodo.bin");
	...
	const std::vector<uint64_t>& globalReps)
{
	...
	for (int depth = 1; depth <= maxDepth; depth++)
	{
		...
		{
			qDebug() << "Time up before starting depth" << depth;
			break;
		}
		...
		if (!timedOut)
		{
			if (anyFullThisDepth)
			{
				...
				qDebug() << "Depth" << depth << "completed fully.";
				qDebug() << " -> Best depth score:" << bestDepthScore;
				qDebug() << " -> Updating bestFull to this depth result.";
			}
			else {
				qDebug() << "Depth" << depth << "completed but NO full-eval moves!?";
			}
			continue;
		}
		...
		// Timeout happened
		// --------------------------------------------
		qDebug() << "TIMEOUT during depth" << depth;

		if (!anyFullThisDepth)
		{
			qDebug() << "No full-evaluated moves in this depth, keeping previous bestFull.";
			break;
		}
		...
		if (!haveFull || bestDepthScore > bestFullScore)
		{
			...
			qDebug() << "Timeout but deeper full result is BETTER.";
			qDebug() << " -> Updating bestFull.";
		}
		else
		{
			qDebug() << "Timeout: deeper full result is WORSE.";
			qDebug() << " -> Keeping previous depth's best.";
		}
	...
	qDebug() << "----------";
	qDebug() << "FINAL best move score =" << bestFullScore;
...
bool Engine::loadOpeningBook(const std::string& filename)
{
	...
	if (!f) {
		qDebug() << "Book load: fopen failed:" << QString::fromStdString(filename);
		return false;
	}
	...
	while (fread(buf, 1, 16, f) == 16) {
		...
		// debug print first few entries (helpful)
		if (count < 8) {
			qDebug().nospace()
				<< "BOOK[" << count << "] key=0x"
				<< QString::number((qulonglong)e.key, 16)
				<< " move=0x" << QString::number(e.move, 16)
				<< " weight=" << e.weight
				<< " learn=" << e.learn;
		}
	...
	if (openingBook.empty()) {
		qDebug() << "Book load: no entries loaded.";
		return false;
	}
	qDebug() << "Book loaded entries:" << (int)openingBook.size();
	return true;
...
Move Engine::probeBook(const board& b)
{
	...
	qDebug() << "ProbeBook: looking up key 0x" << QString::number((qulonglong)key, 16);
	...
	qDebug() << "Chosen random book move from top" << take
		<< ": move16=0x"
		<< QString::number(chosen->move, 16)
